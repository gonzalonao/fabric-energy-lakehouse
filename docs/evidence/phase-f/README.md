...
| File | What it proves |
|---|---|
| `f6-prod-items-published.png` | **The full deploy landed.** `ws-energy-prod` listed flat across folders (the `Location` column shows `/silver`, `/orchestration`, `/gold`), with every deployed item present: `env_energy`, `lh_energy`, 7 notebooks, 4 pipelines, `sm_energy`, `rpt_energy`, `vl_energy` — **16 items**, matching the deploy script's count exactly. The 17th row is `lh_energy`'s **SQL analytics endpoint**, which Fabric creates automatically alongside a lakehouse and which is therefore *not* a deployed definition — a useful reminder that a workspace item count and a deploy count are not the same number. `sm_energy` and `rpt_energy` also carry a `Refreshed` timestamp from the post-fix reframe (see F7). *(Fabric has no expand-all for folders; searching `_` — present in every item name — returns the flat cross-folder list seen here.)* |
